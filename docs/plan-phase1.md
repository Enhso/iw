...
- a handcrafted 1-entity payload with every list empty except entities ingests fine (empty `<- $x` inputs must work; if mnestic rejects an empty constant relation, skip that block by building the script dynamically from the non-empty parts and document that in the module docs).
